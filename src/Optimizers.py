...
def Adam_desc(l_rate,Nstep,energy_fn,Init_R):
    tx=optax.adam(learning_rate=l_rate)
    params=Init_R
    opt_state = tx.init(params)
    Obj_fn=energy_fn
    Obj_grad_fn=jax.jit(jax.value_and_grad(Obj_fn))
    Traj=jnp.zeros((Nstep,*Init_R.shape))
    Traj=Traj.at[0].set(params)
    def step_fn(i,packed_state_Traj):
        params,Traj,opt_state,=packed_state_Traj[0],packed_state_Traj[1],packed_state_Traj[2]
        Val,grad=Obj_grad_fn(params)
        updates, opt_state = tx.update(grad, opt_state,params)
        params = optax.apply_updates(params, updates)
        Traj=Traj.at[i].set(params)
        return params,Traj,opt_state
    return jax.lax.fori_loop(1,Nstep,jax.jit(step_fn),(params,Traj,opt_state))
