...
def CSH(key :KeyArray,
    ...
    #4 : Create random configuration/ Read from file
    
    #R = box_size * random.uniform(key, (N, spatial_dimension), dtype=jnp.float64)
    Data_CSH=pd.read_table("structure.dat",sep='\s+',skiprows=15,nrows=N,header=None)
    R=Data_CSH.to_numpy()[:,4:7]
    ...
    return N_species,box_size, species,R,Disp_Vec_fn,pair_dist_fn,Node_energy_fn,Total_energy_fn,jax.jit(displacement_fn), jax.jit(shift_fn) 
