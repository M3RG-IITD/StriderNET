...
class Pol_Net(nn.Module):
    ...
    MLP2x_layers:int  #MLP2 for x  displacement
    MLP2y_layers:int  #MLP2 for y displacement
    
    train : bool
    
    #MLP2y_layers: int #MLP2 
    ...
    def setup(self):
        ...
        MLP2x_features=[self.node_emb_size]*self.MLP2x_layers+[1] #[x,y]
        MLP2y_features=[self.node_emb_size]*self.MLP2y_layers+[1] #[x,y]
        ...
        self.MLP2x_mlp=[nn.Dense(feat) for i,feat in enumerate(MLP2x_features)]
        self.MLP2y_mlp=[nn.Dense(feat) for i,feat in enumerate(MLP2y_features)]
    ...
    def __call__(self, graphs: jraph.GraphsTuple):
        ...
        def Displace_node(G):
            ...
            x=f_vec
            for i, lyr in enumerate(self.MLP2x_mlp):
                x = lyr(x)
                x = self.MLP2_activation(x)
                
            #Pass f_vec to MLPy
            y=f_vec
            for i, lyr in enumerate(self.MLP2y_mlp):
        
                y = lyr(y)
                y = self.MLP2_activation(y)
                
            r_vec=0.1*jnp.concatenate([x,y],axis=1)
            return r_vec 
