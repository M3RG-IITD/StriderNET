...
class Generic_system:
    def create_batched_States(self,key :KeyError,System='LJ',spatial_dimension: int=3,N :int=38, N_sample :int=100,Batch_size: int=10):
        """
        ...
        #FIRE_desc(1e-8,1000,R,Total_energy_fn,shift_fn)[0].position  #Initial minimize for 1000 steps
        
        #Min_R=FIRE_desc(1e-8,1000,R,Total_energy_fn,shift_fn)[0].position  #Initial minimize for 1000 steps
        #4:NVT_Liquid Initialisation
        key3, key4 = jax.random.split(key1,2)
            
        if(System=='LJ'):
            Traj=jnp.array(onp.load("DatasetLJ100_3D_Pos.npy"))[:100,:,:]
            N_sample=100
            print(Batch_Total_energy_fn(Traj))
        else:
            _,log=NVT_Liq_Init(key4,2.0,1e-4,N_sample*10,10,Min_R,Total_energy_fn,shift_fn)
            Traj=log['position']
        ...
        return Train_Batch,Val_Batch,Test_Batch, shift_fn,Batch_pair_cutoffs,Batch_pair_sigma,Batch_Disp_Vec_fn,Batch_Node_energy_fn,Batch_Total_energy_fn
    ...
    def create_G(self,R,species,pair_cutoffs,pair_sigma,Disp_Vec_fn,Node_energy_fn,Total_energy_fn):
        ...
        dr_pair =jax_md.space.distance(dR)
