...
class Pol_Net(nn.Module):
    edge_emb_size:int
    node_emb_size: int
    fa_layers: int   #fa MLP has additional batchnorm layer
    fb_layers: int
    fv_layers: int
    fe_layers: int
    ...
    fa_activation: Callable[[jnp.ndarray], jnp.ndarray] =jax.nn.leaky_relu
    fb_activation: Callable[[jnp.ndarray], jnp.ndarray] =jax.nn.leaky_relu
    fv_activation: Callable[[jnp.ndarray], jnp.ndarray] =jax.nn.leaky_relu
    fe_activation: Callable[[jnp.ndarray], jnp.ndarray] =jax.nn.leaky_relu
    MLP1_activation: Callable[[jnp.ndarray], jnp.ndarray] =jax.nn.leaky_relu
    MLP1_normalize_activation: Callable[[jnp.ndarray], jnp.ndarray] = nn.softmax
    MLP2_activation: Callable[[jnp.ndarray], jnp.ndarray] = jax.nn.leaky_relu
    ...
    def __call__(self, graphs: jraph.GraphsTuple):
        ...
        def update_edge_fn(edges, senders, receivers, globals_):
            '''
            #Senders[i]=node feature of sender of edge 'i', Similarly  for receivers
            '''
        ...
        def update_node_fn(nodes, sent_edges, received_edges, globals_): #fv
            '''
            #sent_edges[i]=agg_sum_of outgoing edges for node 'i'

            '''
            x = jnp.concatenate([nodes,received_edges,sent_edges],axis=1) #concatenates edge feature to each node
        ...
        def Displace_node(G):
            ...
            #Scaling of predicted displacements  
            r_vec=0.01*self.sigma*x  
            r_vec=r_vec-jnp.mean(r_vec,axis=0)  #Remove the pure translation component
            return r_vec 
